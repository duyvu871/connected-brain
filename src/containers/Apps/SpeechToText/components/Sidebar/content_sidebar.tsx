...
const ContentSidebar = forwardRef<HTMLDivElement, ContentSidebarProps>((props, ref) => {
	...
	return (
		<Box
			ref={ref}
			className={cn(
				'flex-grow h-full py-5 pl-5 flex flex-col justify-center items-center',
				className,
			)}
			{...others}>
			<Center className={'w-full rounded-2xl  mb-5'}>
				<Flex
					...
					<div className='p-2'>
						<Image
							className='w-20 h-20 rounded-full mx-auto color-gray-400'
						...
						<Tab
							...
							title={
								<Flex justify={'space-between'} align={'center'} className={'gap-2'}>
									<p
										className={
											'w-full opacity-0 group-hover:opacity-100 absolute right-[50%] top-1/2 transform -translate-y-1/2 translate-x-full group-hover:translate-x-1/2 transition-all duration-[500] ease-in-out text-white'
										}>
										{item.title}
									</p>
									<Box
										className={
											'absolute right-1/2 top-1/2 transform -translate-y-1/2 translate-x-1/2 group-hover:right-0 transition-all duration-[500] ease-in-out'
										}>
										{item.icon}
									</Box>
								</Flex>
