						...
						{!isAssistant &&
							contentMedia.map((media, index) => (
								<div className={'relative w-24 h-28'} key={'content-media_' + generateUID()}>
									<div
										className={
											'w-full h-full relative rounded-xl flex justify-center items-center bg-gray-800 overflow-hidden'
										}>
										<Image
											src={media}
											radius={'lg'}
					...
					{isAssistant && (
						<>
							<Tooltip title={'Like this response'}>
								<div
									className={
										'p-2 rounded-full bg-zinc-800 cursor-pointer transition-all hover:bg-zinc-700 hover:text-white'
									}>
									<AiOutlineLike />
								</div>
							</Tooltip>
...
export default function MessageListRender() {
	...
	return (
		<>
			<div
				className={
					' flex flex-col items-center gap-5 overflow-hidden overflow-y-auto w-full h-full pt-10 pb-20 relative'
				}
				ref={scrollRef}>
				{isNewSection ? (
					<LaunchScreen />
