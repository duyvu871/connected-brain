...
function FeatureItemRender({ itemList = [] }: { itemList: typeof featuresList1 }) {
				...
				>
					...
					<div className={'h-fit border-0 border-t-[1px] border-gray-900'}>
						...
						>
							...
							<motion.span
								className={cn('text-xs text-gray-300 h-11 line-clamp-3 shadow')}
								animate={{
									transition: {
										delay: 0.1,
									},
								}}>{feature.description}</motion.span>
						</motion.div>
						<div
							className={'backdrop-blur-[6px] h-0 w-full absolute z-10 bottom-0 transition-all showCardHoverContent'}></div>
					</div>
				</div>
			))}
		</>
...
function ShowCard({}: ShowCardProps) {
	// const [currentHover, setCurrentHover] = React.useState<string>('');
	return (
