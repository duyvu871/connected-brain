...
const UserButton = ({
											username, email, avatar, setCollapse = () => {
		},
										}: {
		username: string;
		email: string;
...
export default function UserSidebar({ isOpen, setIsOpen }: UserSidebarProps) {
	...
	const mainLinks = links.map((link) => (
		<div key={link.label} className={'flex justify-between items-center'}>
			...
			{link.notifications && (
				<div className={'w-4 h-4 rounded-full bg-blue-400 text-white text-xs flex place-content-center'}>
					{link.notifications}
				</div>
			)}
		</div>
			...
			>
				...
				<div className={'px-2'}>
					<Input
						...
						startIcon={
							<div
								className={'p-1 bg-gray-600 rounded-lg content-center my-2 mr-2 cursor-pointer hover:opacity-80 transition-all'}>
